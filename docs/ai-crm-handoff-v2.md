...
- 成员之间第一期不能互看邮件正文。
- 组织管理员可看组织内线索、序列和统计。
- 组织管理员是否可看成员邮件正文已由 CRM 组织权限配置决定，默认不可看。
- 组织管理员可暂停/停止成员序列。
- 组织管理员不能编辑正文、确认草稿、代发、代回复。
- 平台超管可查看邮件正文，客户无感；查看非本人线程正文会写静默审计，不强制填写原因。
- 普通系统日志不记录邮件正文。
...
51. 发送队列/同步日志运维薄视图
    ...
    - 运维概览已补详情抽屉：发送队列侧展示状态、客户、联系人、脱敏邮箱、主题、步骤、线程方式、job、runVersion、Gmail thread 和时间；同步侧展示脱敏邮箱、负责人、授权状态、watch、checkpoint、同步问题和时间。
...
- 组织级权限配置页已支持“管理员查看成员邮件正文”开关，默认关闭。
- 收件箱详情按组织配置返回成员邮件正文；普通成员仍只能看本人正文。
- 平台超管查看非本人收件箱正文会写 `inbox-body-viewed-by-super-admin` 静默审计，日志 metadata 不写正文。
...
- 前端 CRM 配置页支持组织邮件模板库 CRUD，每个模板组维护第 1-5 封模板并可设置默认模板。
- `getTemplateDefaults` 在组织默认模板存在时返回组织模板，否则返回内置全局默认模板。
...
- 组织级权限配置已完成：管理员查看成员邮件正文由组织开关控制，超管查看非本人正文写静默审计。
