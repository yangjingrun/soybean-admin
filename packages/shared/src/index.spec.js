...
describe('shared permissions', () => {
  ...
  it('does not expose personal provider channels as assignable role permissions', () => {
    ...
    assert.equal(crmPermissionDefinitions.map(item => item.group).includes('ai_settings_model'), false);
    assert.equal(crmPermissionDefinitions.map(item => item.group).includes('ai_settings_serper'), false);
    assert.equal(crmPermissionDefinitions.map(item => item.group).includes('ai_settings_hunter'), false);
