...
export function parseKeywordOptimizationPlan(text) {
  return JSON.parse(text);
}
/** Converts raw AI finish reasons into user-facing status text. */
export function formatAiFinishReason(reason) {
...
export function createAiResultFromKeywordHistory(record) {
  return {
...
export function buildKeywordHistoryUpdatePayload(requirement, keywordPlan) {
  return {
...
export function cloneKeywordPlan(plan) {
  return JSON.parse(JSON.stringify(plan));
}
/** Checks whether a target lead count can be sent to the search workflow. */
export function isValidTargetLeadCount(value) {
...
export function createKeywordOptimizationViewModel(plan, isSuperAdmin) {
  return {
    summaryItems: [
...
export function formatKeywordOptimizationVisibleText(viewModel) {
...
export function buildAiLeadCandidateImportRows(candidates) {
...
export function normalizeAiLeadCandidateDomain(candidate) {
  const website = candidate.website?.trim();
  if (!website) {
    return null;
  }
...
function normalizeQueryRow(query) {
  return {
...
function readQueryMetaValue(query, key) {
  return query.meta?.[key] || query[key] || '';
}
function buildAiLeadCandidateImportState(candidate, index, firstIndexByDomain) {
...
function collectFirstIndexByDomain(candidates) {
  const firstIndexByDomain = new Map();
...
function isObviousLowQualityCandidate(candidate) {
...
function compactSourceSnapshot(record) {
...
function getPlacesQueries(plan) {
