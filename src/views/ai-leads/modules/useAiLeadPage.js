...
export function useAiLeadPage() {
  ...
  const searchTaskActionSuccessTextMap = {
    interrupt: '采集任务已中断',
    resume: '采集任务已继续',
    retry: '采集任务已重试',
    discard: '采集任务已放弃',
  ...
  const searchTaskPollTimer = shallowRef(null);
  const deletingKeywordHistoryId = shallowRef('');
  const aiResult = shallowRef(null);
  const searchProgress = ref(createLeadSearchProgressState());
  const currentSearchTask = shallowRef(null);
  const keywordQualityWarnings = ref([]);
  const historyRecords = ref([]);
  const editableKeywordPlan = ref(null);
  const editingKeywordPlanSnapshot = ref(null);
  const selectedHistoryId = shallowRef('');
  const isTargetLeadCountTouched = shallowRef(false);
  const keywordResultOrigin = shallowRef('none');
  ...
  function applyKeywordHistoryRecord(record, options = {}) {
  ...
  function applySearchTaskRecord(task, options = {}) {
  ...
  function handleTargetLeadCountUpdate(value) {
  ...
  async function syncCurrentSearchTaskAfterRequestError(options = {}) {
  ...
  function clearHandledSearchTaskContext(action) {
    resetSearchProgress();
    if (action !== 'read') {
      return;
    }
  ...
  function startSearchTaskPolling(taskId) {
  ...
  async function refreshSearchTask(taskId) {
  ...
  function runSearchTaskAction(action, taskId) {
    const actionMap = {
  ...
  function notifySearchTaskStatusChange(task) {
    if (task.status === 'completed') {
