...
const pendingReplyTarget = {
  routePath: '/crm/inbox',
  query: { status: 'pending' }
};
const sentTodayTarget = {
...
const mailboxIssueTarget = {
  routePath: '/crm/settings'
};
const missingContactTarget = {
  routePath: '/crm/leads',
...
const startAiLeadTarget = {
  routePath: '/ai-leads'
};
/** Pick the single action users should handle first on today's workbench. */
export function buildWorkbenchRecommendation(overview) {
...
export function buildWorkbenchMetricCards(overview) {
  const today = overview?.today;
...
export function buildWorkbenchTodoItems(overview) {
  const today = overview?.today;
  if (!today) return [];
  const items = [];
  pushTodo(items, {
...
export function buildWorkbenchTrendOption(trend) {
  return {
    tooltip: {
      trigger: 'axis'
    },
...
export function shouldPollWorkbench(overview) {
  return Boolean(overview?.runningTasks.some(task => ['queued', 'running'].includes(task.status)));
}
export function formatWorkbenchUpdatedAt(value) {
...
export function formatTaskCountMeta(task) {
