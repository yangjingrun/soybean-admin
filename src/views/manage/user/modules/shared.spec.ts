...
describe('system user shared helpers', () => {
  ...
  it('reads expiration and lock display state from row flags', () => {
    assert.deepEqual(getUserExpirationState({ expired: false, expireAt: null } as Api.SystemUser.UserListItem), {
      label: '长期有效',
      description: '',
      type: 'success'
    });
    ...
    assert.deepEqual(
      ...
      } as Api.SystemUser.UserListItem),
      {
        label: '有效至',
    ...
    assert.deepEqual(getUserLockedState({ locked: false, lockedUntil: null } as Api.SystemUser.UserListItem), {
      label: '正常',
      description: '',
      type: 'success'
    });
