...
import { defaultAiPromptKey } from '@/constants/ai-gateway';
import {
...
export function usePromptSettingsPage() {
  ...
  const selectedPromptKey = shallowRef(defaultAiPromptKey);
  ...
  const latestTestRun = shallowRef(null);
  const validatedPromptText = shallowRef('');
  const focusSectionRequest = shallowRef(null);
  ...
  const rollingBackVersionId = shallowRef(null);
  ...
  async function loadSteps(preferredPromptKey = selectedPromptKey.value) {
  ...
  async function loadDetail(promptKey = selectedPromptKey.value) {
  ...
  function focusPromptSection(key) {
