...
const validationItemSectionMap = {
...
export function resolvePromptStepStatus(step) {
  if (step.draft) {
    return {
      label: '有草稿',
...
export function summarizePromptValidation(result) {
  if (!result) {
    return {
      ok: false,
      passCount: 0,
...
export function buildPromptSectionAnchors(systemPrompt) {
...
export function resolvePromptLineStartOffset(systemPrompt, line) {
  if (line <= 1) {
    return 0;
  }
...
export function resolvePromptValidationSection(item) {
...
export function formatLatestPromptTestRunForCopy(run) {
  if (!run) {
    return '';
  }
  const lines = [`测试结果：${run.success ? '通过' : '失败'}`];
  if (run.errorMessage) {
...
export function resolvePromptPublishBlockReason(state) {
