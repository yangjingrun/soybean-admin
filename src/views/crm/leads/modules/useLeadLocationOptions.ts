...
interface CountryOption extends SelectOption {
  label: string;
  value: string;
  raw: Api.Crm.GeoCountryOption;
}

interface CityOption extends SelectOption {
  label: string;
  value: string;
  raw: Api.Crm.GeoCityOption;
}

/** Manage country/city cascading options for the lead import form. */
export function useLeadLocationOptions(formModel: Ref<Api.Crm.LeadImportFormModel>, visible: Ref<boolean>) {
  ...
  function handleCityChange(cityName: string | null) {
