...
export function useEmailSequenceTable() {
  ...
  const batchNextDraftResultDisplays = shallowRef([]);
  ...
  const filterModel = reactive(createDefaultSequenceFilterModel());
...
function getRouteQueryNumber(value) {
  const rawValue = getRouteQueryString(value);
  const parsedValue = Number(rawValue);
  return Number.isInteger(parsedValue) && parsedValue > 0 ? parsedValue : null;
}
function isSequenceEnrollmentStatus(value) {
  return [
...
function isSequenceReviewTodoType(value) {
  return [
    'draft_review_pending',
...
function isSequenceReviewCreatedAtScope(value) {
