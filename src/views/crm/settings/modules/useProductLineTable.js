...
export function useProductLineTable() {
  ...
  const operatingProductLineId = shallowRef(null);
  const editingProductLineId = shallowRef(null);
  const editingProductLineRecord = shallowRef(null);
  ...
  const filterModel = reactive(createDefaultProductLineFilterModel());
  const formModel = reactive(createDefaultProductLineForm());
  ...
  async function handleSubmitProductLine() {
    ...
    try {
      ...
      if (isEdit && !canManageAiWritingConfig.value) {
        delete payload.aiWritingConfig;
      }
  ...
  async function handleArchiveProductLine(record) {
