...
export function useSequencePolicyTable() {
  ...
  const operatingPolicyId = shallowRef(null);
  const editingPolicyId = shallowRef(null);
  ...
  const filterModel = reactive(createDefaultSequencePolicyFilterModel());
  const formModel = reactive(createDefaultSequencePolicyForm());
  ...
  async function handleArchiveSequencePolicy(record) {
  ...
  async function handleSetDefaultSequencePolicy(record) {
