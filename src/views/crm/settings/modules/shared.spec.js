...
function createSequenceReviewItem(options) {
  return {
    account: { name: options.accountName },
    canControlSequence: true,
    canOperateDraft: true,
    checklist: [],
    contact: { email: options.contactEmail, fullName: '' },
    enrollment: {
      runVersion: 3,
      status: options.enrollmentStatus ?? 'ready_to_send'
    },
    firstMessage: null,
    mailbox: { maskedEmail: 'm***@example.com' },
...
function createSystemLog(options) {
...
function createAiDraftTaskConfigRecord(options = {}) {
  return {
    configKey: 'global',
    itemConcurrency: 3,
    maxItemConcurrency: 5,
...
function createAiDraftTaskRecord(options = {}) {
  return {
    id: 'ai-draft-task-1',
...
function createEmailTemplateGroup() {
  return {
    id: 'template-1',
    organizationId: 'org-1',
...
function createEnabledAiWritingConfig() {
  return {
    enabled: true,
    commonRequirements: 'Natural English',
    forbiddenClaims: 'No fake certificates',
...
function createStrategyStatRow(input = {}) {
  return {
    dimension: 'template',
    key: 'default_template',
    name: '默认模板',
