...
import {
  closePendingGmailOAuthTab,
  openGmailOAuthUrlInTab,
  openPendingGmailOAuthTab,
  type GmailOAuthTabHandle,
  type GmailOAuthTabOpen
} from './mailbox-oauth-tab';

function createTab(): GmailOAuthTabHandle & { closed: boolean } {
  return {
    closed: false,
    location: {
      href: ''
...
describe('mailbox OAuth tab helpers', () => {
  it('opens a pending blank tab from the click event and clears opener', () => {
    ...
    const openTab: GmailOAuthTabOpen = (url, target, features) => {
