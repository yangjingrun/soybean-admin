...
export function useBlacklistTable() {
  ...
  const removingRecord = shallowRef(null);
  const removeFormModel = reactive({
    reason: ''
  });
  ...
  const filterModel = reactive(createDefaultBlacklistFilterModel());
  ...
  function openRemoveModal(record) {
    if (!canRemove.value) {
      return;
    }
  ...
  function handleRemoveModalVisibleUpdate(show) {
