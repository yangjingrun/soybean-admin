...
export function useInboxThreadDetail() {
  ...
  const currentDetail = shallowRef(null);
  const detailLoading = shallowRef(false);
  const replyTopic = shallowRef('');
  const replyBody = shallowRef('');
  const replyPolishUndoSnapshot = shallowRef(null);
  ...
  const selectedThreadId = shallowRef(getRouteParamString(route.params.id));
  ...
  async function loadThreadDetail(id = selectedThreadId.value, options = {}) {
    if (!id) {
      return;
    }
  ...
  async function markPendingThreadHandledOnOpen(detail) {
  ...
  function syncReplyDraftFromDetail(detail) {
  ...
  async function handleConfirmUnsubscribe(messageId) {
    const threadId = selectedThreadId.value;
...
function getRouteParamString(value) {
