...
interface LeadSearchStreamOptions {
  signal?: AbortSignal;
  baseURL?: string;
}

/** Streams AI leads search progress events from the backend. */
export async function streamLeadCustomerSearch(
  data: Api.AiLeads.SearchOrchestratePayload,
  handlers: Api.AiLeads.LeadSearchStreamHandlers,
  options: LeadSearchStreamOptions = {}
) {
  try {
...
function getDefaultBaseURL() {
  const env = (import.meta.env || {}) as Env.ImportMeta;
